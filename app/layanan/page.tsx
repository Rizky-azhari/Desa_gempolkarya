      ...
      <section className="py-16 md:py-24 bg-eggshell dark:bg-background">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <Tabs defaultValue="all" className="w-full mt-4 flex flex-col items-center">
            {/* Tabs Trigger Header */}
            <TabsList className="bg-cardSoft dark:bg-card border border-orinoco/40 dark:border-border/60 rounded-2xl p-1 mb-10 overflow-x-auto max-w-full flex justify-start sm:justify-center gap-1">
              {categories.map((cat) => (
                <TabsTrigger
                  key={cat.value}
                  value={cat.value}
                  className="rounded-xl px-4 py-2 text-xs sm:text-sm font-semibold text-textMuted data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm font-sans flex items-center gap-1.5 cursor-pointer"
                >
                  {cat.icon}
                  <span>{cat.label}</span>
                </TabsTrigger>
